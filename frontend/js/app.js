...
async function initApp() {
    ...
    if (hashStr.includes("reset-password") || hashStr.includes("type=recovery") || hashStr.includes("access_token") || hashStr.includes("recovery=true")) {
...
function checkAdminAccess(actionName = "do that") {
    if (!state.user?.is_admin) {
        showToast(`Only Admin can ${actionName}`, "error");
        return false;
    }
    return true;
}
...
function applyRBACUI() {
    ...
    // Show admin-only buttons (Assign Admin, Invite Others) only to Admin users
    ...
    sidebarNavItems.forEach(item => {
        ...
        if (href === "#dashboard") {
            ...
            // Milestones and Upload Documents are admin-only management views
    ...
    // Hide team management controls for non-admin users
    const teamControls = document.getElementById("team-management-controls");
    if (teamControls) {
        teamControls.style.display = isAdmin ? "flex" : "none";
    }

    // Hide delete project button for non-admin users
    const deleteProjectBtn = document.getElementById("btn-delete-project");
    if (deleteProjectBtn) {
        deleteProjectBtn.style.display = isAdmin ? "inline-flex" : "none";
    }

    // Hide "New Project" button for non-admin users
    const newProjectBtn = document.getElementById("btn-open-create-project-modal");
    if (newProjectBtn) {
        newProjectBtn.style.display = isAdmin ? "inline-flex" : "none";
    }

    // Hide "Add Milestone" button for non-admin users
    const addMilestoneBtn = document.getElementById("btn-add-milestone-direct");
    if (addMilestoneBtn) {
        addMilestoneBtn.style.display = isAdmin ? "inline-flex" : "none";
    }

    // Hide "Upload Document" button for non-admin users
    const addDocumentBtn = document.getElementById("btn-add-document-direct");
    if (addDocumentBtn) {
        addDocumentBtn.style.display = isAdmin ? "inline-flex" : "none";
    }

    // Hide "Add Team Member" button for non-admin users
    const addTeamMemberBtn = document.getElementById("btn-add-team-member");
    if (addTeamMemberBtn) {
        addTeamMemberBtn.style.display = isAdmin ? "inline-flex" : "none";
    }
...
async function loadDashboardStats() {
    ...
    try {
        ...
        // If a project is selected, filter logs to only show entries related to that project
        ...
        if (selectedProjId) {
            ...
            const mRes = await fetch(`${API_BASE}/api/milestones/project/${selectedProjId}`, {
                headers: { "Authorization": `Bearer ${state.token}` }
            });
            const docsRes = await fetch(`${API_BASE}/api/documents/project/${selectedProjId}`, {
                headers: { "Authorization": `Bearer ${state.token}` }
            });
            ...
            for (let p of projs.slice(0, 5)) {
                const mRes = await fetch(`${API_BASE}/api/milestones/project/${p.id}`, {
                    headers: { "Authorization": `Bearer ${state.token}` }
                });
                const docsRes = await fetch(`${API_BASE}/api/documents/project/${p.id}`, {
                    headers: { "Authorization": `Bearer ${state.token}` }
                });
                if (mRes.ok) totalMilestones += (await mRes.json()).length;
                if (docsRes.ok) totalDocs += (await docsRes.json()).length;
        ...
        milestones.forEach(milestone => {
            ...
            card.innerHTML = `
                ...
                <div class="timeline-card-header">
                    ...
                    <div class="timeline-card-actions" style="display: ${state.user?.is_admin ? 'flex' : 'none'};">
...
async function loadProjectDetailDocuments(projectId) {
    ...
    try {
        ...
        // Get project milestones to display names
        const mRes = await fetch(`${API_BASE}/api/milestones/project/${projectId}`, {
            headers: { "Authorization": `Bearer ${state.token}` }
        });
                ...
                <td>
                    ...
                    <div style="display: flex; gap: 6px; align-items: center;">
                        <button class="btn btn-secondary btn-sm" id="btn-gen-stories-${doc.id}"
                            ...
                            <i data-lucide="sparkles" style="width: 14px; height: 14px;"></i> Stories
                        </button>
    ...
    if (messages.length === 0) {
        container.innerHTML = `
            <div class="chat-bubble bot-message">
                ...
                <div class="message-content">
                    Ready to chat! Ask me anything about the selected project's documents.
                </div>
            </div>
        `;
    } else {
...
async function loadActivityLogs() {
    ...
    try {
        ...
        // If a global project is selected, filter logs to only show entries for that project
        const selectedProjId = state.globalProjectId || "";
...
async function populateProjectDropdowns() {
    ...
    try {
        ...
        if (effectiveGlobal) {
            ...
            // No global — restore individual saved values
            if (valUpload) uploadProjSelect.value = valUpload;
            if (valChat) chatProjSelect.value = valChat;
            if (valFilter) filterProjSelect.value = valFilter;
            if (valCreate) createMilestoneSelect.value = valCreate;
            if (valStory && storyProjSelect) storyProjSelect.value = valStory;
        }
...
if (inviteForm) inviteForm.addEventListener("submit", async (e) => {
    ...
    const password = document.getElementById("invite-password-input").value.trim();
    ...
    try {
        ...
        inviteForm.reset();
        document.getElementById("invite-password-input").value = "Welcome123!";
    ...
    panel.innerHTML = `
        ...
        <div style="display: flex; flex-wrap: wrap; gap: 28px; align-items: flex-start;">
            ...
            <div style="flex: 1 1 440px; min-width: 0;">
                <!-- Summary / Title -->
                <textarea ${isAdmin ? '' : 'readonly'} onchange="updateStoryField(${projectId}, ${story.id}, 'title', this.value)" rows="1" style="font-size: 1.4rem; font-weight: 700; color: var(--color-text-main); background: transparent; border: 1px solid transparent; width: 100%; padding: 6px 8px; border-radius: 6px; resize: none; overflow: hidden; line-height: 1.35; font-family: inherit; transition: border-color 0.15s, background 0.15s; margin-bottom: 24px;" onfocus="this.style.border='1px solid var(--border-color)'; this.style.background='var(--bg-card)';" onblur="this.style.border='1px solid transparent'; this.style.background='transparent';">${story.title}</textarea>
                ...
                <div style="margin-bottom: 28px;">
                    ...
                    <textarea ${isAdmin ? '' : 'readonly'} onchange="updateStoryField(${projectId}, ${story.id}, 'description', this.value)" placeholder="Add a description..." style="width: 100%; min-height: 90px; background: var(--bg-card); border: 1px solid var(--border-color); border-radius: 8px; padding: 12px; color: var(--color-text-main); font-size: 0.95rem; line-height: 1.5; resize: vertical; font-family: inherit;">${story.description || ''}</textarea>
                </div>
                ...
                <div style="border-top: 1px solid var(--border-color); padding-top: 24px;">
                    ...
                    <div style="display: flex; gap: 10px;">
                        ...
                        <button type="button" onclick="addStoryComment(${projectId}, ${story.id})" class="btn btn-primary" style="padding: 8px 16px; font-size: 0.9rem; display: flex; align-items: center; gap: 6px; font-weight: 600;">
                            <i data-lucide="send" style="width: 14px; height: 14px;"></i> Save
                        </button>
                    </div>
                </div>
            </div>
