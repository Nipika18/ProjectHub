...
def generate_stories_from_single_document(
    ...
    The admin clicks a button next to a document to trigger this.
    """
    project = db.query(Project).filter(Project.id == project_id).first()
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
...
def get_project_stories(
    ...
    stories = db.query(UserStory).filter(UserStory.project_id == project_id).order_by(priority_order, UserStory.id.desc()).all()
...
def update_story(
    ...
    if not current_user.is_admin:
        ...
        if ("title" in request.model_fields_set or 
            ...
            raise HTTPException(status_code=403, detail="Only administrators can edit user story details.")
...
def update_task(
    ...
    if not current_user.is_admin:
        ...
        if "title" in request.model_fields_set or "task_type" in request.model_fields_set or "assigned_to" in request.model_fields_set or "due_date" in request.model_fields_set:
            raise HTTPException(status_code=403, detail="Only administrators can edit task details (title, type, assignee, or due date).")
